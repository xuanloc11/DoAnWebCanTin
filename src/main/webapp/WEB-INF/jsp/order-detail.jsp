    ...
    <c:choose>
        ...
        <c:otherwise>
            ...
            <c:choose>
                ...
                <c:when test="${order.trangThaiOrder eq 'DA_HUY' || order.trangThaiOrder eq 'CANCELLED'}">Đã hủy</c:when>
            ...
            <div class="card">
                ...
                <div class="actions">
                    <a class="btn btn-ghost" href="${pageContext.request.contextPath}/profile#orders">← Quay lại danh sách đơn</a>
                    <c:if test="${order.trangThaiOrder eq 'MOI_DAT' || order.trangThaiOrder eq 'DA_XAC_NHAN'}">
                        <form action="${pageContext.request.contextPath}/profile/order/cancel" method="post" style="display:inline-block; margin-left:8px;" onsubmit="return confirm('Bạn có chắc chắn muốn hủy đơn hàng này?');">
                            <input type="hidden" name="order_id" value="${order.orderId}" />
                            <button type="submit" class="btn btn-danger" style="font-size:13px; padding:8px 14px;">Hủy đơn hàng</button>
                        </form>
                    </c:if>
                </div>
            </div>
